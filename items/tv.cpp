...
Tv::Tv(float x, float y, float z, float r, float l, float w, float h, const Texture &t) : Item (x, y, z, r) , length(l), texture(t), height(h), collisionMatrix(), width(w)
{
...
void Tv::generateList()
{
	...
	glBegin(GL_QUADS);
		...
		quickpoint(0, (height / 2), 0);
		quickpoint(0, height, 0);
		quickpoint(length, height, 0);
		quickpoint(length, (height / 2), 0);
...
bool Tv::isCollision(float x, float y, float z)
{
